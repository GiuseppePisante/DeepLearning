...
class RNN(BaseLayer):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.layers = [
            ...
            FullyConnected(hidden_size, hidden_size),
    ...
    def forward(self, input_tensor):
        ...
        output_tensor = np.zeros((B, T, self.output_size))
        hidden_state = np.zeros((B, self.hidden_size))

        for t in range(T):
            self.states.append([None] * 4)
            self.states[t][0] = input_tensor[:, t, :]
            self.states[t][1] = hidden_state

            hidden_state = self.layers[0].forward(input_tensor[:, t, :])
            hidden_state = self.layers[1].forward(hidden_state)
            self.states[t][2] = hidden_state

            output_tensor[:, t, :] = self.layers[2].forward(hidden_state)
            self.states[t][3] = output_tensor[:, t, :]
    ...
    def backward(self, error_tensor):
        ...
        self.layers[0].gradient_weights = FC1_weights
        self.layers[2].gradient_weights = FC2_weights
